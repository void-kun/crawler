...
class RarSpider(scrapy.Spider):
    ...
    def media_parse(self, res):
        try:
            download_link = res.xpath('//div[@id="download_link"]//a[@id="downloadButton"]').attrib['href']
    ...
    def item_parse(self, res):
        try:
            media_url = res.xpath("//div[@class='entry-content single-page']//p//a[@rel='nofollow noopener']").attrib['href']
